...
`kiro-auth add` is a guided flow: it saves the active Kiro CLI login, shows an interactive `Open Browser (Easy)` / `Manual / Incognito` chooser, asks for confirmation, logs out the single active Kiro CLI session, launches the selected native `kiro-cli login` flow, and imports the new login when Kiro finishes. Manual mode suppresses browser auto-open as much as Kiro CLI allows and copies the manual URL/code to the clipboard. Use `kiro-auth sync` when you only want to import the currently active Kiro CLI session without logging out.
