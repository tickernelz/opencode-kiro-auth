...
function readKiroCliEmail(): string | undefined {
...
function readAccounts(): AccountRow[] {
...
function upsertAccount(account: AccountRow): void {
  const dbPath = getPluginDbPath()
...
function updateAccount(id: string, patch: Partial<AccountRow>): AccountRow {
...
function getAccountByIndex(indexText: string | undefined): { account: AccountRow; index: number } {
...
function statusFor(account: AccountRow): string {
...
function accountLine(account: AccountRow, index: number): string {
...
function listAccounts(): CommandResult {
  const accounts = readAccounts()
...
function addCurrentKiroCliAccount(): CommandResult {
...
function writeAccountToKiroCli(account: AccountRow): void {
  const cliDbPath = getKiroCliDbPath()
  if (!existsSync(cliDbPath)) throw new Error(`Kiro CLI database not found: ${cliDbPath}`)
  const cliDb = openDatabase(cliDbPath)
  try {
...
function switchAccount(indexText: string | undefined): CommandResult {
...
function enableAccount(indexText: string | undefined, enabled: boolean): CommandResult {
  const { account, index } = getAccountByIndex(indexText)
...
function resetAccount(indexText: string | undefined): CommandResult {
  const { account, index } = getAccountByIndex(indexText)
...
function removeAccount(indexText: string | undefined): CommandResult {
  const { account, index } = getAccountByIndex(indexText)
